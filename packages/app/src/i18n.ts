...
const zh = {
  ...
  quota: {
    ...
    subtitle:
      "同一套 kimi-code 三窗真实数据(rolling_5h + weekly + monthly, requests 计数制), 分别用 3 种卡头×三窗空间关系渲染。**三窗 QuotaMeter(layout=row) 常驻直显**是信息主体, BarRowTooltip 仅作可选密度增强(行 hover 补 micro 视图, 非信息载体)。差异落在真排版维度, 不是头部装饰件堆叠。",
    open: "查看方案页",
    legendOk: "ok(健康)",
    legendWarn: "warn(偏低)",
    legendBad: "bad(耗尽)",
    ...
    // (QuotaMeter row 三件套装不进 ~104px 列宽), 留 3 个真维度方案: 空间结构(P1)/信息层级(P2)/头部承载(P4)
    cardP1Name: "Provider 卡 · P1 基线竖排(主页同构)",
    cardP1Desc:
      "卡头 = handle + 名称 + StatusDot + 状态徽章(一行)。三窗 row 各一行 QuotaMeter, 窗间 8px gap。**与主页 ProviderCard/BarsTemplate 1:1 完全一致, 认知零成本**。",
    cardP2Name: "Provider 卡 · P2 头部综合态(信息上抬, 无摘要条)",
    cardP2Desc:
      "卡头右侧合并「StatusDot + 综合态文字」一行(整卡 health 一瞥可读); 三窗 row 同 P1。**不引入摘要条形态**——风险颜色仍走行内自身 color, 头部不抽警示带。",
...
const en: Dict = {
  ...
  quota: {
    ...
    subtitle:
      "The same kimi-code three-window real data (rolling_5h + weekly + monthly, requests count), rendered in 3 head × three-window spatial relations. **Three-window QuotaMeter (layout=row) is always-on and direct** — that's the card's information main body. BarRowTooltip is only an optional density enhancement (row hover reveals a micro view, not an information carrier). Differences live in real layout dimensions, not in head decoration stacking.",
    open: "View gallery",
    legendOk: "ok (healthy)",
    legendWarn: "warn (low)",
    ...
    // (QuotaMeter row's three-piece set doesn't fit in a ~104px column). Keep 3 real-dimension schemes: spatial (P1)/info hierarchy (P2)/head carrying (P4)
    cardP1Name: "Provider card · P1 baseline vertical (home page isomorphic)",
    cardP1Desc:
      "Head = handle + name + StatusDot + status badge (one row). Three windows as QuotaMeter(row) each on its own line, 8px gap between windows. **1:1 identical with the home page ProviderCard/BarsTemplate, zero cognitive cost**.",
    cardP2Name: "Provider card · P2 head carries rollup (info elevation, no summary strip)",
    cardP2Desc:
      "Head right side merges 'StatusDot + rollup label' into one line (whole-card health at a glance); three windows row, same as P1. **No summary strip** — risk color stays on each row's own color, the head does not extract a warning band.",
