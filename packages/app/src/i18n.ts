...
const zh = {
  ...
  set: {
    ...
    themeHint:
      "默认追随系统外观, 可在此覆盖。左下角 ☀ 钮可快切浅色/深色/跟随系统, 与此处三档同走一套主题。",
    sort: "排序",
    sortName: "名称",
    sortUrgency: "紧要度",
    sortManual: "手动",
    ...
    sortDesc: "倒排",
    sortHint:
      "缺省按名称正排。紧要度 = 按卡内最紧窗口的剩余比例排序(剩余越少越靠前), 方向独立生效。\n            手动 = 拖动卡片自定义顺序, 此时方向不适用。",
    autostart: "开机自启",
  ...
  quota: {
    title: "额度组件实例方案",
    subtitle:
      "「最小组件」完整四元素排版: 标题 + 重置时间 + 进度条 + 用量。同一排版喂不同数据(pct 状态色 / 条形态 / 标题 / 重置), 竖排逐个展示多个完整实例, 非表格。全部走 tokens.css / 8px 网格 / D-016 三态(dark/light/glass)。",
    open: "查看方案页",
    iFlash: "闪购 40 次",
    iDeep: "深度 58 次",
    iWeek: "周窗 72 次",
...
const en: Dict = {
  ...
  quota: {
    title: "Quota component instances",
    subtitle:
      "Complete 4-element minimal component: title + reset time + progress bar + usage. Same layout fed different data (pct state tint / bar shape / title / reset), shown as stacked individual instances, not a table. All via tokens.css / 8px grid / D-016 three states (dark/light/glass).",
    open: "View gallery",
    iFlash: "Flash 40 uses",
    iDeep: "Deep 58 uses",
    iWeek: "Weekly 72 uses",
    iMonth: "Monthly 91 uses",
    iPerf: "Perf 2300 uses",
    iResetSoon: "resets soon",
    iResetWeek: "resets in 5.8d",
    iResetDayFrac: "resets in 3.4d",
    iResetHours: "resets in 6.4h",
    iResetDayInt: "resets in 12d",
    legendOk: "ok (healthy)",
    legendWarn: "warn (low)",
