...
test("bars+ticker 模板: 进度条/倒计时/最紧标红 + 余额预计可用天数", async ({ hostPage, page }) => {
  ...
  // micro 排版短格式(t_f7d1beeb 9/7): micro quota-usage 只显百分比, 不带 requests/次 等单位标签
  // (kimi mock rolling_5h ≈ 82%, Math.round 后显 "82%")
  await pwExpect(kimiBars.locator(".bar-row > .quota-meter > .quota-usage").first()).toContainText("82%");
