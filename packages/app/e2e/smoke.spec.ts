...
test("混合场景: 最坏情况置顶 + 异常卡文字占位", async ({ tauriPage, page }) => {
  void tauriPage;
  await page.getByTestId("consent-agree").click();
  ...
  // 最坏情况优先: auth_expired 卡置顶
  await pwExpect(cards.first()).toHaveAttribute("data-health", "bad");
  await pwExpect(cards.first()).toContainText("登录态过期");
  // window 卡有手写进度条(D-002)
  await pwExpect(page.locator(".progress").first()).toBeVisible();
});
