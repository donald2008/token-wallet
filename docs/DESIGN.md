...
1. 卡片排序可配置(#829 R1 + D-039, 2026-08-30 真机反馈): key(名称|紧要度|手动)×dir(正排|倒排)两正交参数, 缺省名称正排;
   紧要度 = 卡内最紧窗口剩余比例升序(剩余越少越靠前), 健康度三级序降级为紧要度的并列稳定键;
   **手动(D-039)**: 拖动卡片 head 左侧品牌色块(16px 拖动手柄)即接管排序 —— 任何模式直接拖卡立即切 manual,
   按 config.order(providerId 数组)排, **以当前实例集合为准做交集**(order 里没有的 id 按缺省规则名称正排追加尾部,
   order 里的幽灵 id 忽略 —— order 永远是附加信息, 实例集合是真相源, 不因 order 过期丢卡);
   manual 忽略 dir(固定 asc); 切到名称/紧要度时 order 保留不清(再切回手动恢复自定义顺序);
   拖动过程只更新内存 state, **松手(drop)才持久化一次**(settings.json + localStorage 降级, 走 sortConfig 通道);
   设置页排序控件三档(名称/紧要度/手动), manual 时方向控件禁用;
   托盘全局最差状态不受排序配置影响
...
- **重置倒计时刻度**(#829 R2): 单单位 + 一位小数(≥1天 → X.X天 / <1天 → X.X小时 / <1小时 → X分整数), 纯倒计时无动作后缀;
  倒计时列固定宽右对齐, 多行不漂移
- **设置弹窗布局**(#829 R3): 头部(标题+关闭)固定不滚, 滚动只发生在头下内容区, 且内容区滚动不穿透背景面板(overscroll contain)
...
```
左侧窄功能侧栏(44px 常驻, D-038): ＋ 添加 / ⟳ 刷新 / (弹性空隙) / ⚙ 设置
标题栏(D-038 瘦身): 全局状态点 / app-title / 图钉置顶 / 最小化 / 关闭 —— 全部常显, 无 hover 显隐
云端卡片区: 排序可配置(名称|紧要度 × 正排|倒排, 缺省名称正排, #829 R1), 原型模板渲染;
           卡 head 右上删除钮(hover 卡片淡入 → 确认气泡)
本地 Agent 区(默认折叠): per-agent 用量 + 云×本地对比行
```

### 6.6 操作分区(信息架构, D-038)

provider 余额/额度展示是主体, 操作按**作用域**分区, 一个位置一职:

| 分区 | 作用域 | 承载动作 | 形态 |
|------|--------|---------|------|
| 左侧窄侧栏 | 全局 | 添加 provider / 手动刷新 / 打开设置 | 44px 垂直常驻, 手绘 SVG 图标(D-002) + title tooltip, hover/active 用 `--bg-hover`/`--accent` |
| provider 卡片 | 单实例 | 删除该实例 | 卡 head 右上删除钮, hover 卡片淡入, 点击弹既有 confirm-delete 气泡(含取消, 红调) |
| 设置弹窗 | 偏好 | 主题 / 排序 / 开机自启 / 存储路径展示 | 模态弹窗, head 固定 + body 滚动(#829 R3) |
| 标题栏 | 窗口 | 置顶 / 最小化 / 关闭 + 全局状态点 | 单行常显, 不承载功能动作 |

判据: 全局动作进侧栏(低频但要随手可达), 实例动作就近放卡片(所见即所改), 偏好进设置(不与动作混排),
标题栏只留窗口控制 —— 宽度预算让给标题与状态(360px 下曾因六个工具钮把标题挤到断词撑高)。

- **添加向导**(选平台 → 填 key)为独立组件, 双入口: 侧栏 ＋(弹窗) / 空态大按钮(页内导航, D-021 首开引导不变);
  保存成功即关向导回面板, 新卡直接出现(不再回落"实例列表")。
- **设置页不再承载 provider 管理**: 无添加入口、无实例列表、无实例删除钮。
- **hover 显隐已废除**: 标题栏控件从 6 个减到 3 个后常显更可预测(隐藏控件是发现性成本), 图钉不再需要"置顶时常显"特判。
