...
>
> **v0.2.8 同步**（2026-09-08）：D-051 已将「最坏情况优先排序」收敛为「手动拖拽排序唯一默认」，
> 三档命名排序 UI 已删；本契约与 DECISIONS.md v0.2.8 系列决策（D-048 ~ D-053）配套使用。
>
> 配套权威来源：`docs/DESIGN.md`（设计原理）、`docs/DECISIONS.md`（决策记录 D-001 ~ D-053）、
...
主形态 = 360×720 紧凑面板（v0.2.8 起；D-038 旧托盘+弹出 + t_27eeadad 720 高度均已被 t_d086543b 收敛），
辅形态 = 桌面悬浮窄条（可最小化到托盘）。

**第一设计原则 = Glanceability（一瞥可读，DESIGN.md §6.1）**：

1. **手动排序 = 唯一默认**（D-051, 2026-09-07 收敛）：拖动卡片 = 用户接管排序，松手一次持久化；「最坏情况优先置顶」/「名称/紧要度自动排序」均已废弃，托盘全局最差色点不受影响。
